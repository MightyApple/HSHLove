...
router.post(
  "/signup",
  multer.fields([{ name: "images", maxCount: 6 }]),
  async (req, res) => {
    try {
      ...
      const t = await mongoHSHLove.userDataCollection
        .insertMany([data])
        .then(() => {
          try {
            if (req.files) {
...
router.get("/getImages", async (req, res) => {
  try {
    if (req.session.authorized) {
      const user = await mongoHSHLove.userDataCollection
        .findOne({
          _id: req.session.user._id,
        })
        .then((user) => {
...
router.post("/validateData", async (req, res) => {
  try {
    ...
    if (user == null) {
      ...
      res.send({
        message: "Nutzer mit der Email existiert bereits",
        noError: false,
      });
    }
  } catch (e) {
